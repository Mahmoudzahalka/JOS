...
copy_shared_pages(envid_t child)
{
	...
	//lab5-start-ex8
	if(child < 0)
		panic("copy_shared_pages: envid_t child < 0 \n");
	uint32_t address = 0;
	for(address = 0; address < USTACKTOP; address+=PGSIZE)       //the threshoold ??
	{
		if((uvpd[PDX(address)]&PTE_P) && (uvpt[PGNUM(address)]&PTE_P) && (uvpt[PGNUM(address)]&PTE_SHARE))
		{
			int r;
			if ((r = sys_page_map(sys_getenvid(), (void *)address, child, (void *)address, uvpt[PGNUM(address)]&PTE_SYSCALL)) < 0)
				return r;
		}
	}
	//lab5-end-ex8
