...
bc_pgfault(struct UTrapframe *utf)
{
	...
	if(r<0)
	{
		panic("bc_pgfault: sys_page_alloc fails \n");
	}
	r = ide_read(blockno*BLKSECTS, addr, BLKSECTS);
	if(r < 0)
	{
		panic("bc_pgfault: ide_read fails \n");
	}
	//lab5-end-ex2
...
flush_block(void *addr)
{
	...
	int r;
	addr = (void *)ROUNDDOWN(addr,PGSIZE);
	if(va_is_mapped(addr) == true && va_is_dirty(addr) == true)
	{
		...
		if (r < 0)
		{
			panic("in flush_block, sys_page_map fails \n");
		}
	}
	//lab5-end-ex2
