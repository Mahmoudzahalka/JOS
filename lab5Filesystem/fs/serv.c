...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	// Second, call the relevant file system function (from fs/fs.c).
	// On failure, return the error code to the client
	int num_req = req->req_n;
	if(req->req_n > PGSIZE)
		num_req = PGSIZE;
	if((r = file_read(o->o_file, ret->ret_buf, num_req, o->o_fd->fd_offset))<0)
		return r;
	o->o_fd->fd_offset += r;
	return r;
	//lab5-end-ex5
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	// On failure, return the error code to the client
	int num_req = req->req_n;
	if(req->req_n > PGSIZE)   // maybe should be : PGSIZE - (sizeof(int) + sizeof(size_t))
		num_req = PGSIZE;
	if((r = file_write(o->o_file, req->req_buf, num_req, o->o_fd->fd_offset)) <0)
		return r;
	o->o_fd->fd_offset += r;
	return r;
	//lab5-end-ex6
